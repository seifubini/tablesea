                            ...
                            <table id="example1" class="table table-bordered table-striped">
                                ...
                                @foreach ($memberships as $membership)
                                    <tr>
                                        ...
                                        <td>{{ $membership->Restaurant_name}}</td>
                                        <td>
                                            {{ date_format(date_create($membership->created_at), 'jS M Y')}}
                                        </td>
                                        @if(Auth::user()->user_type == "Restaurant")
                                            <td>
                                                <button type="button" title="delete" style="border: none; background-color:transparent;"
                                                        data-toggle="modal" data-target="#modal-danger{{$membership->id}}">
                                    ...
                                    @endif
                                        ...
                                        <!-- Modal Starts Here-->
                                        <div class="modal fade" id="modal-danger{{$membership->id}}">
                                            <div class="modal-dialog">
                                                <div class="modal-content bg-danger">
                                                    <div class="modal-header">
                                                        <h4 class="modal-title">Are you sure you want to delete <br>
                                                            <strong>"{{ $membership->membership_name }}"</strong> ?</h4>
                                                    </div>

                                                    <form action="{{ route('delete_membership', $membership->id) }}" method="GET">
